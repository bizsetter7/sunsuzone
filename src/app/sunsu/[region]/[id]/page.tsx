...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    if (normalizedSlug) {
        ...
        return {
            ...
            openGraph: {
                title,
                description,
                url: `https://www.sunsuzone.kr/sunsu/${encodeURIComponent(decodedRegionSlug)}/${encodeURIComponent(normalizedSlug)}`,
                siteName: '선수존',
                images: [{ url: 'https://www.sunsuzone.kr/og-image.jpg', width: 1200, height: 630, alt: '선수존' }],
                type: 'website',
            },
            alternates: {
                canonical: `https://www.sunsuzone.kr/sunsu/${encodeURIComponent(decodedRegionSlug)}/${encodeURIComponent(normalizedSlug)}`,
            },
        };
    }
    ...
    return {
        ...
        openGraph: {
            title,
            description,
            url: `https://www.sunsuzone.kr/sunsu/${encodeURIComponent(slugify(shop.region))}/${shop.id}`,
        ...
        alternates: {
            canonical: `https://www.sunsuzone.kr/sunsu/${encodeURIComponent(slugify(shop.region))}/${shop.id}`,
        },
...
export async function generateStaticParams() {
    ...
    // 2. 지역×업종 가이드 랜딩 페이지 (예: /sunsu/서울/호스트바선수)
...
export default async function ShopDetailPage({ params }: Props) {
    ...
    if (normalizedId) {
        ...
        const breadcrumbSchema = {
            ...
            itemListElement: [
                ...
                { '@type': 'ListItem', position: 2, name: `${regionName} 알바`, item: `https://www.sunsuzone.kr/sunsu/${decodedRegionSlug}` },
                { '@type': 'ListItem', position: 3, name: `${regionName} ${workTypeInfo.name}`, item: `https://www.sunsuzone.kr/sunsu/${decodedRegionSlug}/${normalizedId}` },
            ],
        };
    ...
    const jsonLd = {
        ...
        "hiringOrganization": {
            "@type": "Organization",
            "name": shop.name,
            "sameAs": `https://www.sunsuzone.kr/sunsu/${slugify(shop.region)}/${shop.id}`
        },
