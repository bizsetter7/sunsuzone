        ...
        <div className={`space-y-5 animate-in fade-in duration-300`}>
            ...
            <div className={`p-6 md:p-8 rounded-[32px] border shadow-sm ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-blue-50'}`}>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className={`p-4 rounded-2xl flex items-center gap-3 ${isDark ? 'bg-gray-800' : 'bg-amber-50'}`}>
                    ...
                    <button
                        ...
                        className={`w-full py-5 rounded-[24px] font-black text-lg transition-all ${
                            todayChecked
                                ...
                                : 'bg-gradient-to-r from-[#D4AF37] to-pink-400 text-white shadow-xl shadow-amber-200 hover:brightness-110 active:scale-[0.98]'
                        }`}
                    >
                        {todayChecked
                            ? '✅ 오늘 출석 완료!'
            ...
            <div className={`p-5 md:p-8 rounded-[32px] border shadow-sm ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
                    ...
                    {['일', '월', '화', '수', '목', '금', '토'].map((d, i) => (
                        <div
                            ...
                            className={`text-center text-[11px] font-black py-1 ${
                                i === 0 ? 'text-amber-400' : i === 6 ? 'text-blue-400' : 'text-gray-400'
                            }`}
                        >
                            {d}
                        </div>
                ...
                {isLoading ? (
                    <div className="flex items-center justify-center py-16">
                        <div className="w-8 h-8 border-4 border-amber-200 border-t-[#D4AF37] rounded-full animate-spin" />
                    </div>
                ) : (
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`relative flex flex-col items-center justify-center aspect-square rounded-xl mx-0.5 my-0.5 transition-all text-sm font-black
                                                ${isAttended ? 'bg-amber-500 text-white shadow-md shadow-amber-200' : ''}
                                                ${isToday && !isAttended ? 'ring-2 ring-[#D4AF37] ring-offset-1' : ''}
                                                ${isFuture ? 'opacity-30' : ''}
                                                ${di === 0 && !isAttended ? 'text-amber-400' : ''}
                                        ...
                                        >
                                            ...
                                            {isToday && !isAttended && (
                                                <div className="absolute bottom-1 w-1 h-1 rounded-full bg-[#D4AF37]" />
                                            )}
                                        </div>
                                    );
                                })}
                ...
                <div className="flex items-center gap-4 mt-5 pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex items-center gap-1.5 text-xs font-bold text-gray-400">
                        <div className="w-4 h-4 rounded-md border-2 border-[#D4AF37]" />
                        오늘
                    </div>
                </div>
            </div>
