        ...
        <div className={`min-h-screen will-change-scroll ${brand.theme === 'dark' ? 'bg-gray-950 text-white' : 'bg-gray-50 text-gray-800'} ${isCorporateModalOpen ? 'overflow-hidden h-screen pointer-events-none' : ''}`}>
            ...
            <div className="flex md:hidden items-center px-4 py-5">
                <h2
                    ...
                    className={`text-xl font-black text-[#D4AF37] transition-all duration-300 ${activeTab === '프리미엄 라운지' && searchParams.get('lounge') ? 'cursor-pointer active:scale-95' : ''}`}
                >
                    {activeTab}
                </h2>
            </div>
            ...
            {activeTab !== '프리미엄 라운지' && (
                <div className="bg-gradient-to-r from-[#D4AF37] to-amber-400 p-6 text-white relative overflow-hidden shadow-xl">
                    <div className="max-w-[1200px] mx-auto relative z-10">
                        ...
                        <div className="relative z-20">
                            ...
                            <button onClick={() => handleTabChange('전체')} className="bg-white text-[#D4AF37] px-6 py-3 rounded-2xl font-black text-sm shadow-xl shadow-amber-900/20 active:scale-95 transition-all outline-none">
                                수다 떨러 가기
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="max-w-[1200px] mx-auto rounded-2xl bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 px-4 py-3.5 flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-black text-amber-800 mb-2">
                                포인트를 모으는 방법!&nbsp;
                                <span className="text-[#D4AF37]">가입만 해도 600P!</span>
                            </p>
                                ...
                                <div className="flex items-center gap-x-2 md:gap-x-4 flex-nowrap">
                                    {[
                                        ...
                                        <span key={label} className="text-[10px] md:text-[11px] text-amber-700 font-bold whitespace-nowrap">
                                            ✅ {label} <span className="text-[#D4AF37] font-black">{point}</span>
                                        </span>
                                    ))}
                                </div>
                                {/* Row 2: 게시글, 댓글, 출석체크 */}
                            ...
                            <div className="flex justify-between items-center mb-4 px-4 sm:px-0">
                                ...
                                {userType !== 'corporate' && (
                                    <button
                                        ...
                                        className="bg-[#D4AF37] text-slate-900 px-5 py-2.5 rounded-2xl font-black text-sm shadow-lg shadow-amber-200 hover:bg-amber-500 active:scale-95 transition-all flex items-center gap-2"
                                    >
                                        <PenLine size={18} /> 글쓰기
                                    </button>
                                )}
                                    ...
                                    return (
                                        <React.Fragment key={post.id}>
                                            <div
                                                ...
                                                className={`p-6 sm:rounded-[32px] shadow-sm border active:scale-[0.98] transition-all cursor-pointer hover:border-amber-200 group ${brand.theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}
                                            >
                                                <div className="flex justify-between items-start mb-3">
                                                    <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-[11px] font-black group-hover:bg-amber-50 group-hover:text-[#D4AF37] transition-colors">
                                                        {post.category}
                                                    </span>
                                                ...
                                                <div className="flex items-center justify-between text-xs border-t border-gray-50 pt-5">
                                                    <div className="flex items-center gap-2">
                                                        <div className="w-8 h-8 bg-amber-50 rounded-full flex items-center justify-center text-[#D4AF37] shadow-inner">
                                                            <User size={16} />
                                                        </div>
                                                        <span className={`font-black ${brand.theme === 'dark' ? 'text-gray-300' : 'text-gray-800'}`}>{post.author}</span>
                                                    </div>
                                                    <div className="flex gap-5">
                                                        <span className="flex items-center gap-1.5 text-[#D4AF37] font-black">
                            ...
                            {filteredPosts.length > 0 && (
                                <div className="flex justify-center items-center gap-1.5 md:gap-2 mt-12 mb-8">
                                    <button
                                        ...
                                        className={`h-8 md:h-10 px-3 md:px-4 rounded-lg md:rounded-xl font-bold transition-all flex items-center justify-center ${currentPage === 1 ? 'text-gray-300' : 'text-[#D4AF37] hover:bg-amber-50'}`}
                                    >
                                        이전
                                    </button>
                                    {[...Array(totalPages)].map((_, i) => (
                                        <button
                                            ...
                                            className={`w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl font-black transition-all flex items-center justify-center ${currentPage === i + 1 ? 'bg-[#D4AF37] text-slate-900 shadow-lg shadow-amber-200' : 'text-gray-400 hover:bg-gray-100'}`}
                                        >
                                            {i + 1}
                                        </button>
                                    ))}
                                    <button
                                        ...
                                        className={`h-8 md:h-10 px-3 md:px-4 rounded-lg md:rounded-xl font-bold transition-all flex items-center justify-center ${currentPage === totalPages ? 'text-gray-300' : 'text-[#D4AF37] hover:bg-amber-50'}`}
                                    >
                                        다음
                                    </button>
                                </div>
                    ...
                    <div className="fixed inset-0 z-[20000] flex items-center justify-center px-4">
                        ...
                        <div className={`rounded-[32px] md:rounded-[45px] w-[90%] md:w-full max-w-sm p-8 md:p-12 relative z-10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] border ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-white/20'}`}>
                            <div className="flex flex-col items-center text-center">
                                <div className="w-16 h-16 md:w-24 md:h-24 bg-amber-50 rounded-full flex items-center justify-center mb-6 md:mb-10 text-[#D4AF37] ring-8 ring-amber-50 shadow-inner">
                                ...
                                <p className={`mb-8 md:mb-12 leading-relaxed font-bold text-xs md:text-sm ${brand.theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                                    이곳은 <span className="text-yellow-600 font-black underline underline-offset-4 decoration-4">인증된 남성 회원</span>들만<br />
                                ...
                                <div className="grid grid-cols-1 w-full gap-3">
                                    <button
                                        ...
                                        className="w-full py-4 md:py-5 bg-[#D4AF37] text-slate-900 rounded-2xl md:rounded-3xl font-black text-base md:text-lg hover:bg-amber-500 hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-amber-200 outline-none"
                                    >
                                        지금 가입하고 확인하기
                                    </button>
                                    <button
                ...
                mounted && isCorporateModalOpen && createPortal(
                    ...
                    `}>
                            <div className="flex flex-col items-center text-center gap-6">
                                <div className={`p-4 rounded-full ${brand.theme === 'dark' ? 'bg-gray-700/50' : 'bg-amber-50'}`}>
            ...
            {/* Floating Action Button (Always visible on mobile for better accessibility) */}
            {
                userType !== 'corporate' && activeTab !== '프리미엄 라운지' && (
                    <button
                        onClick={handleWriteClick}
                        className="fixed bottom-24 right-5 md:right-10 bg-[#D4AF37] text-slate-900 p-3.5 rounded-full shadow-xl hover:bg-amber-500 active:scale-90 transition-all z-50 hover:shadow-amber-300/50"
                    >
                        <PenLine size={20} />
                    </button>
                )
