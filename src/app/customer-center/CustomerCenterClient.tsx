                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <aside className="w-full md:w-64 shrink-0 z-50 self-stretch relative">
                        <StickyWrapper offsetTop={56} isInternal={true}>
                                    ...
                                    {TABS.map((tab) => (
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-start gap-3 md:gap-4 px-4 py-3 md:px-6 md:py-5 text-[13px] md:text-sm font-black whitespace-nowrap rounded-lg md:rounded-none md:border-l-4 ${activeTab === tab.id
                                                ? `bg-amber-50 md:bg-gradient-to-br md:border-[#D4AF37] shadow-sm md:shadow-none ${brand.theme === 'dark' ? 'from-amber-900/20 to-gray-800 text-amber-400 bg-gray-700' : 'from-amber-50 to-white text-[#D4AF37]'}`
                                        ...
                                        >
                                            <div className={` ${activeTab === tab.id ? 'text-[#D4AF37]' : 'text-gray-300'}`}>
                                                {tab.icon}
                                            </div>
                                            <span>{tab.id}</span>
                                        </button>
                            ...
                            <div className={`hidden md:block mt-1 p-5 rounded-[32px] border ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700 shadow-amber-900/10' : 'bg-white border-gray-100 shadow-amber-100/10'} shadow-xl`}>
                                <div className="flex items-center gap-3 mb-5">
                                    <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-white shadow-lg bg-[#D4AF37]">
                                        <PhoneCall size={20} />
                                    </div>
                                ...
                                <p className={`text-[13px] leading-relaxed font-black ${brand.theme === 'dark' ? 'text-gray-200' : 'text-gray-500'}`}>
                                    평일 10:00 ~ 18:00<br />
                                    점심 12:00 ~ 13:00<br />
                                    <span className="text-[#D4AF37] font-black mt-1 block">공휴일 / 주말 휴무</span>
                                </p>
                    ...
                    <div className="flex-1 min-w-0 pb-20">
                        ...
                        <div className={`md:hidden mt-6 p-5 rounded-[30px] border shadow-xl shadow-amber-100/10 ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gradient-to-br from-white to-amber-50/30 border-amber-100'}`}>
                            ...
                            <p className={`text-[12px] leading-relaxed font-black ${brand.theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>
                                ...
                                <span className="text-[#D4AF37] font-black mt-1 block">공휴일 / 주말 휴무 (텔레그램 상시 대기)</span>
                            </p>
                            <a href="https://t.me/sunsujone_cs_bot" className="mt-6 flex items-center justify-center gap-3 w-full py-4 bg-[#D4AF37] text-white rounded-[20px] text-sm font-black hover:bg-[#b8922e] transition shadow-xl shadow-amber-100">
                                <MessageCircle size={18} /> 텔레그램 실시간 상담
                            </a>
                        </div>
                    </div>
