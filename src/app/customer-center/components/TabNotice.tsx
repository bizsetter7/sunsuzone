...
export const TabNotice = () => {
    ...
    useEffect(() => {
        fetch('/api/notices?platform=sunsuzone')
            .then(r => r.json())
                ...
                <div key={notice.key} className={`${idx !== notices.length - 1 ? (brand.theme === 'dark' ? 'border-b border-gray-700' : 'border-b border-gray-100') : ''}`}>
                    ...
                    >
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                            <span className={`w-12 h-6 flex items-center justify-center shrink-0 rounded text-[10px] font-black ${notice.category === '필독' ? 'bg-red-600 text-white' : notice.category === '공지' ? 'bg-gray-900 text-white' : notice.category === '점검' ? 'bg-gray-400 text-white' : 'bg-[#D4AF37] text-white'}`}>
                                {notice.category}
                            </span>
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-4">
                            ...
                            <div className={`transition-transform duration-300 ${expandedNotice === notice.key ? 'rotate-180 text-[#D4AF37]' : 'text-gray-300'}`}>
                                <ChevronDown size={20} />
                            </div>
                        </div>
                    </div>
