    ...
    return (
        <div className="space-y-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={i} className={`p-6 rounded-[30px] border text-center relative overflow-hidden group hover:shadow-xl transition-all ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                            <span className={`absolute -top-3 -left-3 text-5xl font-black transition-colors pointer-events-none ${brand.theme === 'dark' ? 'text-gray-700' : 'text-gray-50'} group-hover:text-amber-50/50`}>{item.step}</span>
                            <div className="w-14 h-14 bg-amber-50 text-[#D4AF37] rounded-2xl flex items-center justify-center mx-auto mb-5 relative z-10 shadow-inner">
                                {item.icon}
                            </div>
            ...
            <section>
                ...
                <div className={`p-8 md:p-10 rounded-[45px] border shadow-xl shadow-amber-100/10 space-y-10 ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-100 text-gray-900'}`}>
                    <div className="flex flex-col md:flex-row items-center gap-8">
                        <div className="w-20 h-20 bg-amber-50 text-[#D4AF37] rounded-[28px] flex items-center justify-center shrink-0 border border-amber-100">
                            <Briefcase size={36} />
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
                        {[
                            ...
                            <div key={i} className={`flex items-start gap-4 md:gap-5 p-6 md:p-0 rounded-3xl border md:border-0 ${brand.theme === 'dark' ? 'bg-gray-700/50 border-gray-700' : 'bg-gray-50 md:bg-transparent border-gray-100'}`}>
                                <span className="text-4xl md:text-5xl font-black text-[#D4AF37]/20 shrink-0 leading-none w-[36px] md:w-12 text-center">{box.num}</span>
