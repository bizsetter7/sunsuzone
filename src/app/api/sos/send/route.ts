...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // [Phase 5] platform_points 테이블에서 선수존 포인트 조회
        ...
        const { error: deductError } = await supabaseAdmin
            .from('platform_points')
            .upsert(
                { user_id: shopId, platform: 'sunsuzone', balance: currentPoints - pointCost, updated_at: new Date().toISOString() },
        ...
        if (logError) {
            ...
            await supabaseAdmin
                .from('platform_points')
                .upsert(
                    { user_id: shopId, platform: 'sunsuzone', balance: currentPoints, updated_at: new Date().toISOString() },
                    { onConflict: 'user_id,platform' }
                );
