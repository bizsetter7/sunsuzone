...
export default function ShopDetailView({
  ...
  useEffect(() => {
    ...
    supabase
      .from('businesses')
      .select('sunsujone_tier, verified_at, is_verified, manager_name, manager_phone, manager_role, name, room_count, floor_area, opened_at, license_url, description')
    ...
    <div className="flex flex-col w-full h-full bg-gray-50 overflow-hidden">
      ...
      <div className="flex-none flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100 z-50">
        ...
        <div className="flex gap-1.5 shrink-0">
          <button onClick={onToggleFavorite} className="p-1.5 rounded-full hover:bg-gray-100 transition-colors">
            <Heart size={18} className={isFavorite ? 'fill-[#D4AF37] text-[#D4AF37]' : 'text-gray-400'} />
          </button>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        ...
        {tier && tier !== 'basic' && tier !== 'free' && (
          <div className={`text-white text-center py-1.5 text-[11px] font-black tracking-widest uppercase ${
            tier.includes('premium-extra') || tier === 'premium_extra' ? 'bg-amber-500' :
        ...
        <div className="mx-4 mb-4 grid grid-cols-2 gap-2">
          {[
            { icon: <MapPin size={13} className="text-[#D4AF37]" />, label: '지역', value: regionFull || shop.region },
      ...
      <div className="flex-none bg-white border-t border-gray-100 px-4 py-3 flex gap-2.5 safe-area-bottom">
        ...
        <button
          ...
          className="flex-[2.5] py-3.5 bg-[#D4AF37] text-white rounded-2xl flex items-center justify-center gap-2 hover:bg-[#b8922e] transition shadow-lg shadow-amber-200 active:scale-95"
        >
