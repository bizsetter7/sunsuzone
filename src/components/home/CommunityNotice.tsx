    ...
    return (
        <div className="grid grid-cols-2 gap-3 md:gap-6 mb-8 md:mb-12">
            ...
            <div className={communityContainerStyle} onClick={() => router.push('/community')}>
                <div className={headerStyle}>
                    <h3 className={titleStyle}>
                        <MessageCircle className="text-[#D4AF37] shrink-0" fill="currentColor" size={18} />
                        커뮤니티
                    </h3>
                ...
                <div className={listStyle}>
                    {recentPosts.map((post) => (
                        <div key={post.id} className="flex items-center gap-2 md:gap-3 group">
                            <span className={iconBoxStyle('text-[#D4AF37]')}>{getCategoryIcon(post.category)}</span>
                            <span className={`${itemStyle} group-hover:text-[#D4AF37] transition-colors`}>{post.title}</span>
                        </div>
                    ))}
                </div>
                <div className="absolute -top-10 -right-10 w-24 h-24 md:w-32 md:h-32 bg-amber-500/10 rounded-full blur-2xl pointer-events-none"></div>
            </div>
            ...
            <div className={noticeContainerStyle} onClick={() => router.push('/customer-center?tab=notice')}>
                <div className={headerStyle}>
                    <h3 className={titleStyle}>
                        <Bell className="text-[#D4AF37] shrink-0" fill="currentColor" size={18} />
                        공지사항
                    </h3>
                ...
                <div className={listStyle}>
                    {NOTICES.slice(0, 3).map((item) => (
                        ...
                        >
                            <span className={iconBoxStyle('text-[#D4AF37]')}>
                                {item.badge === '중요' ? '🚨' : '📢'}
                            </span>
