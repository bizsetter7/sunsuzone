...
> 최종 업데이트: 2026-05-03 | 작성자: 코부장 | **브랜드 URL 정규화: /coco/ → /sunsu/ (SEO-1E)**
...
/sunsu/[region]         → 지역 SEO 랜딩 페이지 (브랜드 일치 — SEO-1E 2026-05-03)
/sunsu/[region]/[id]    → 광고 상세
/jobs                   → 구인 목록
/jobs/[id]              → 구인 상세
/region                 → 지역 목록
/community              → 커뮤니티
...
**⚠️ 주의**: Footer/page.tsx의 링크는 반드시 `/sunsu/` 경로 사용. `/coco/` 경로는 next.config.ts redirects()에서 301로 `/sunsu/`로 매핑되며, 코드에서 직접 `/coco/`를 사용하면 안 됨.
...
5. **`/coco/` 경로 직접 사용 금지** — 브랜드 URL은 `/sunsu/`. 구 `/coco/` URL은 next.config.ts 301 redirect로 자동 매핑됨 (SEO-1E 2026-05-03)
...
- [x] **2026-05-03 SEO-1E**: 브랜드 URL 정규화 `/coco/` → `/sunsu/` (폴더 rename + 21개 파일 치환 + 301 redirect 추가 + placeholder JobPosting 제거)
